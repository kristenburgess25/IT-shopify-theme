...
theme.Drawers = (function() {
  ...
  Drawer.prototype.init = function() {
    let openVariant = document.querySelector(this.config.selectors.openVariant);
    if (openVariant) {
      openVariant.addEventListener("click", this.open.bind(this));
    }
    this.drawer
...
theme.MobileNav = (function() {
  ...
  function init() {
    ...
    if (cache.mobileNavToggle) {
      ...
      cache.mobileNavOverlay.addEventListener("click", (e) => {
  ...
  function openMobileNav() {
    ...
    cache.mobileNavOverlay.classList.add(classes.navOpen);
    cache.mobileNavContainer.classList.add(classes.navOpen);

    // cache.mobileNavOverlay.style.transform = "translateY(0px) translateX(0)";
    cache.mobileNavContainer.style.transform = "translateY(0px) translateX(0)";

    // cache.pageContainer.style.transform =
    //   "translate3d(0, " + cache.mobileNavContainer.scrollHeight + "px, 0)";

    cache.pageContainer.classList.add("no-scroll");
    cache.pageContainer.setAttribute("aria-hidden", "true");
  ...
  function closeMobileNav() {
    ...
    cache.mobileNavOverlay.classList.remove(classes.navOpen);
    ...
    window.removeEventListener("keyup", keyUpHandler);
    window.scrollTo(0, 0);
    cache.pageContainer.classList.remove("no-scroll");
    cache.pageContainer.setAttribute("aria-hidden", "false");
  }
...
theme.Cart = (function() {
  ...
  Cart.prototype = Object.assign({}, Cart.prototype, {
    _setupCartTemplates: function() {
      ...
      // create a template for the quantity inputs
      this.itemQuantityTemplate = this.itemTemplate
        .querySelector("[data-cart-quantity-wrapper]")
        .cloneNode(true);
    ...
    _createBackUpCart: function(cartTable) {
      const cartBackup = [];
      cartTable
        .querySelectorAll("[data-cart-item]:not([data-removed-item-row='true']")
        .forEach(function(lineItem) {
          var cartItem_id = lineItem
            .getAttribute("data-cart-item-key")
            .split(":");

          //save the product link
          var productLink = lineItem.getAttribute("data-cart-item-url");
          if (!productLink) {
            productLink = "";
          }
          // save the reviews widget
          var stars = lineItem.querySelector(
            "[data-product-stars] .trustpilot-widget"
          );
          if (!stars) {
            stars = "";
          }
          var lines = lineItem.querySelector(
            "[data-type-info] .extra-line-type-container"
          );
          if (!lines) {
            lines = "";
          }

          //save the buy more and save message
          var multiUnitDiscount = lineItem.querySelector(
            "[data-multiunit-discount-message]"
          );
          if (!multiUnitDiscount) {
            multiUnitDiscount = "";
          }
          //save the addon block
          var hasAddon = "";
          if (
            lineItem.nextElementSibling &&
            lineItem.nextElementSibling !== undefined
          ) {
            if (
              lineItem.nextElementSibling.hasAttribute("data-product-addon")
            ) {
              hasAddon = lineItem.nextElementSibling;
            }
          }
          // add to backup object
          cartBackup.push({
            variant_id: cartItem_id,
            productLink: productLink,
            stars: stars,
            lines: lines, 
            multiunit_discount: multiUnitDiscount,
            has_addon: hasAddon,
          });
        });
      return cartBackup;
    },
    _restoreBackupCart: function(cartBackup) {
      var cartIndex = 1;

      cartBackup.forEach(function(lineItem, index) {
        var findMatchingItem = document.querySelector(
          "[data-cart-item-index='" + cartIndex + "']"
        );
        ...
        // if the item has been removed, continue with loop without incrementing index
        if (findMatchingItem.hasAttribute("data-removed-item-row")) {
          return;
        }

        // make sure the line item has the correct variant id
        var itemKeyAttribute = findMatchingItem.getAttribute(
          "data-cart-item-key"
        );
        var itemKeyVariantId = itemKeyAttribute.split(":");

        if (itemKeyVariantId[0] != lineItem.variant_id[0]) {
          return;
        }

        // when the matching cart item is found, restore the additional cart pieces

        if (lineItem.stars !== "") {
          var starsContainer = findMatchingItem.querySelector(
            "[data-product-stars"
          );
          if (starsContainer) {
            starsContainer.innerHTML = "";
            starsContainer.appendChild(lineItem.stars);
          }
        };
        if (lineItem.lines !== "") {
          var linesContainer = findMatchingItem.querySelector(
            "[data-type-info"
          );
          if (linesContainer) {
            linesContainer.innerHTML = "";
            linesContainer.appendChild(lineItem.lines);
          }
        };
       

        if (lineItem.multiunit_discount != "") {
          var findPrice = findMatchingItem.querySelector(
            "[data-cart-item-line-price] [data-cart-item-regular-price-group]"
          );

          if (!findPrice) {
            findPrice = findMatchingItem.querySelector(
              "[data-cart-item-line-price] [data-cart-item-discounted-price-group]"
            );
          }

          findPrice.after(lineItem.multiunit_discount);
        }

        if (lineItem.has_addon !== "") {
          findMatchingItem.after(lineItem.has_addon);
        }
    ...
    _createCart: function(state) {
      ...
      var cartBackup = this._createBackUpCart(cartTable);
      cartTable.innerHTML = "";
      ...
      this._restoreBackupCart(cartBackup);
    ...
    _createLineItemList: function(state) {
      return state.items.map(
        function(item, index) {
          ...
          var quantityInputsWrapper = this.itemQuantityTemplate.cloneNode(true);
    ...
    _setItemLinePrice: function(item, price, quantityInputsWrapper) {
      item.querySelector(selectors.cartItemLinePrice).innerHTML =
        price.outerHTML;

      item
        .querySelector(selectors.cartItemLinePrice)
        .appendChild(quantityInputsWrapper);
    },
      ...
      return options.map(
        function(option) {
          ...
          if (option.value == "None") {
            optionElement.classList.add(classes.hide);
          } else {
    ...
    _getPropertyList: function(properties) {
      ...
      return filteredPropertiesArray.map(
        function(property) {
          ...
          if (property[0] == "Province" || property[0] == "Country") {
          ...
          propertyElement.classList.remove(
            classes.hide,
            "product-details__item--property-province"
          );
          propertyElement.classList.add(
            "product-details__item--property-" + property[0].toLowerCase()
          );
    ...
    _onRemoveItem: function(evt) {
      ...
      if (evt.target.hasAttribute("data-cart-update")) {
        updateProduct = true;
  ...
  Product.prototype = Object.assign({}, Product.prototype, {
    ...
    _initAddToCart: function() {
      this.productForm.addEventListener(
        ...
        function(evt) {
          ...
          // Ben and Slava want to trigger this on change instead of on Add To Cart
          // They don't want to put a strong limit of 10, just trigger the modal
          // We can revisit if needed! --Jackie
          // if (this.quantityInput.value > 10) {
          //   evt.preventDefault();
    ...
    _setActiveThumbnail: function(mediaId) {
      ...
      if (typeof mediaId === "undefined") {
        var productMediaWrapper = this.container.querySelector(
          this.selectors.productMediaWrapper + "[data-slick-index='0']"
        );
    ...
    _adjustThumbnailSlider: function(activeThumbnail) {
      ...
      var slideGroupLeaderIndex =
        Math.floor(
          Number(sliderItem.getAttribute("data-slider-slide-index")) / 1
        ) * 1;
